...
`healthz` 表示进程可响应，`readyz` 表示服务已准备接收流量。Prometheus 指标覆盖进程资源、连接、流量、路由健康和 CONNECT 运行状态，包括 CONNECT 建立耗时分布、H2 物理握手及健康 PING 失败（含超时），并限制标签范围，避免暴露客户端请求信息。

Python3 可直接观察当前实际传输使用的 H2/H3 协议、上游线路、活动隧道、瞬时/平均速率和流量占比，并区分主动健康检查的恢复确认与线路冷却、半开状态；H3 线路还会显示 RTT、丢包、阻塞、轮换与恢复状态。脚本会过滤微量后台流量，并对主线路切换做连续采样确认：
